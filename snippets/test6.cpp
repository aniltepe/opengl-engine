...
//        cout << "object " + objPtr->name + " is drawable, processing shader phase" << endl;
        ...
        if (objPtr->type == ObjectType::Framebuffer && objPtr->style.fboType != FboType::shadow) {
            ...
            objPtr->shader.vertexShader += "out VS_OUT { vec2 TexCoord; float TexOrder; float TexQty; } vs_out;\n";
            objPtr->shader.vertexShader += "uniform mat4 model;\n";
            objPtr->shader.vertexShader += "void main() {\n";
            objPtr->shader.vertexShader += "\tvs_out.TexCoord = vTexCoord;\n";
            objPtr->shader.vertexShader += "\tvs_out.TexOrder = vTexOrder;\n";
            objPtr->shader.vertexShader += "\tvs_out.TexQty = vTexQty;\n";
...
//                objPtr->shader.fragmentShader += "\t\t\tvec3 norm;\n";
                objPtr->shader.fragmentShader += "\t\t\tvec3 norm = texture(textures[order], TexCoord).rgb;\n";
...
//            objPtr->shader.fragmentShader += shadows && objPtr->material.normMapIndexes.size() == 0 ? "\tshadow = CalculateShadow(lightIndex);\n" : "";
            ...
            if (objPtr->material.texture) {
                ...
                objPtr->shader.fragmentShader += "\t\t\tspecular += vec4(lights[lightIndex].material.specular, 1.0f) * specStrength * texture(textures[order], TexCoord);\n";
                objPtr->shader.fragmentShader += "\t\telse {\n";
                objPtr->shader.fragmentShader += "\t\t\tambient += vec4(lights[lightIndex].material.ambient, 1.0f) * texture(textures[order], TexCoord) * vec4(modelMaterial.diffuse, 1.0f);\n";
                objPtr->shader.fragmentShader += "\t\t\tdiffuse += vec4(lights[lightIndex].material.diffuse, 1.0f) * diffStrength * texture(textures[order], TexCoord) * vec4(modelMaterial.diffuse, 1.0f);\n";
                objPtr->shader.fragmentShader += "\t\t}\n";
                objPtr->shader.fragmentShader += "\t}\n";
            }
            
            objPtr->shader.fragmentShader += "\tif (ambient == vec4(0.0)) ambient += vec4(lights[lightIndex].material.ambient, 1.0f) * vec4(modelMaterial.ambient, 1.0f);\n";
            objPtr->shader.fragmentShader += "\tif (diffuse == vec4(0.0)) diffuse += vec4(lights[lightIndex].material.diffuse, 1.0f) * diffStrength * vec4(modelMaterial.diffuse, 1.0f);\n";
            ...
            objPtr->shader.fragmentShader += "\t\t\tfloat intensity = clamp((theta - lights[lightIndex].outerCutOff) / epsilon, 0.0, 1.0);\n";
...
////            if (shadows) {
//                objPtr->shader.fragmentShader += "\tfloat shadow = shadows ? CalculateShadow(lightIndex) : 0.0;\n";
...
void processContinuousInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) {
        vector<Object>::iterator it = find_if(objects.begin(), objects.end(), [](Object obj) { return obj.name == "cubesmooth1"; });
