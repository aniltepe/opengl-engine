...
//  OpenGLTest4
//
//  Created by Nazım Anıl Tepe on 03.10.2021.
//
...
int main()
{
    Object* scene = createScene(WORK_DIR + "scene14.sce");
...
//        cout << "object " + objPtr->name + " is drawable, processing shader phase" << endl;
        ...
        objPtr->shader.vertexShader += (objPtr->type == ObjectType::Cubemap) ? "\tTexCoord = vPos;\n" : "";
        objPtr->shader.vertexShader += (objPtr->type == ObjectType::Cubemap) ? "\tvec3 newPos = (vPos + vec3(0.0, 0.5, 0.0)) * vec3(0.5, 0.5, 0.5);\n" : "";
//        objPtr->shader.vertexShader += (objPtr->type == ObjectType::Cubemap) ? "\tvec3 newPos = vPos + vec3(0.0, 0.5, 0.0);\n" : "";
        objPtr->shader.vertexShader += (objPtr->type == ObjectType::Cubemap) ? "\tvec4 pos = projection * view * vec4(newPos, 1.0);\n" : "";
        objPtr->shader.vertexShader += (objPtr->type == ObjectType::Cubemap) ? "\tgl_Position = pos.xyww;\n" : "";
        ...
        if (objPtr->type == ObjectType::Model) {
            ...
            objPtr->shader.fragmentShader += "\t\tfloat near = 0.01;\n";
            objPtr->shader.fragmentShader += "\t\tfloat far = 100.0;\n";
            objPtr->shader.fragmentShader += "\t\tfloat depth = gl_FragCoord.z;\n";
            objPtr->shader.fragmentShader += "\t\tfloat z = depth * 2.0 - 1.0;\n";
            objPtr->shader.fragmentShader += "\t\tfloat linear = (2.0 * near * far) / (far + near - z * (far - near));\n";
            objPtr->shader.fragmentShader += "\t\tfloat linearDepth = linear / far;\n";
            objPtr->shader.fragmentShader += "\t\tresult -= vec4(vec3(0.0), linearDepth);\n";
//            objPtr->shader.fragmentShader += "\tFragColor = vec4(vec3(linearDepth), 1.0);\n";
        ...
        else if (objPtr->type == ObjectType::Cubemap) {
            objPtr->shader.fragmentShader += "in vec3 TexCoord;\n";
            objPtr->shader.fragmentShader += "uniform samplerCube cubemap;\n";
            objPtr->shader.fragmentShader += "void main() {\n";
            objPtr->shader.fragmentShader += "\tFragColor = texture(cubemap, TexCoord);\n";
        }
...
//        cout << "object " + objPtr->name + " is drawable, processing buffer phase" << endl;
        ...
        if (objPtr->type != ObjectType::Text && objPtr->type != ObjectType::Cubemap) {
            ...
            glBufferData(GL_ARRAY_BUFFER, sizeof(float) * objPtr->shader.vertices.size(), &objPtr->shader.vertices[0], GL_STATIC_DRAW);
...
//                    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
            ...
            glBindTexture(GL_TEXTURE_CUBE_MAP, objPtr->material.textures[0]);
...
//                stbi_image_free(data);
//            }
            vector<unsigned char> decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[4]);
            unsigned char *data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
            ...
            stbi_image_free(data);
            decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[3]);
            data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
                glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
            if (nrChannels == 4)
                glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
            stbi_image_free(data);
            decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[5]);
            data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
            ...
            stbi_image_free(data);
            decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[1]);
            data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
            ...
            stbi_image_free(data);
            decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[0]);
            data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
            ...
            stbi_image_free(data);
            decoded = base64Decode(objPtr->objectPtr->material.texturesBase64[2]);
            data = stbi_load_from_memory(&decoded[0], int(decoded.size()), &width, &height, &nrChannels, 0);
            if (nrChannels == 3)
            ...
            glUseProgram(objPtr->shader.shaderID);
            glUniform1i(glGetUniformLocation(objPtr->shader.shaderID, "cubemap"), 0);
        }
...
//        cout << "object " + objPtr->name + " is drawable, processing draw phase" << endl;
        ...
        if (objPtr->type != ObjectType::Text && objPtr->type != ObjectType::Cubemap) {
            ...
            glUniformMatrix4fv(glGetUniformLocation(objPtr->shader.shaderID, "view"), 1, GL_FALSE, value_ptr(cubeview));
            glActiveTexture(GL_TEXTURE0);
        ...
        if (objPtr->type == ObjectType::Cubemap)
            glDepthFunc(GL_LESS);
    }
    for (int i = 0; i < objPtr->subObjects.size(); i++)
