...
//  OpenGLTest4
//
//  Created by Nazım Anıl Tepe on 22.09.2021.
//
...
int main()
{
    Object* scene = createScene(WORK_DIR + "scene12.sce");
...
//    glFrontFace(GL_CW);
    ...
    string stencilShaderVS = "#version 330 core\nlayout (location = 0) in vec3 vPos;\n"
                             "uniform mat4 model;\nuniform mat4 view;\nuniform mat4 projection;\n"
                             "void main() {\n\tgl_Position = projection * view * model * vec4(vPos, 1.0f);\n}";
    string stencilShaderFS = "#version 330 core\nout vec4 FragColor;"
                             "void main() {\n\tFragColor = vec4(0.04, 0.28, 0.26, 1.0);\n}";
    int vertexShader = glCreateShader(GL_VERTEX_SHADER);
    const char *vertexShaderSource = stencilShaderVS.c_str();
    ...
    const char *fragmentShaderSource = stencilShaderFS.c_str();
    ...
    stencilShader = glCreateProgram();
    glAttachShader(stencilShader, vertexShader);
    glAttachShader(stencilShader, fragmentShader);
    glLinkProgram(stencilShader);
    glGetProgramiv(stencilShader, GL_LINK_STATUS, &success);
    if (!success) {
        glGetProgramInfoLog(stencilShader, 512, NULL, infoLog);
...
//    if (objPtr->name == "verticalplane" && !objPtr->hidden)
...
//        cout << "object " + objPtr->name + " is drawable, processing draw phase" << endl;
        ...
        if (objPtr->name == "human") {
            ...
            glUniformMatrix4fv(glGetUniformLocation(stencilShader, "projection"), 1, GL_FALSE, value_ptr(projection));
            glUniformMatrix4fv(glGetUniformLocation(stencilShader, "view"), 1, GL_FALSE, value_ptr(view));
            float scale = 1.01;
            glm::mat4 model = glm::translate(glm::mat4(1.0f), objPtr->transform.position);
            ...
            glm::mat4 rotation = glm::mat4(objPtr->transform.left.x, objPtr->transform.left.y, objPtr->transform.left.z, 0,
                              ...
                              0, 0, 0, 1);
            model *= rotation;
            glUniformMatrix4fv(glGetUniformLocation(stencilShader, "model"), 1, GL_FALSE,  value_ptr(model));
            glBindVertexArray(objPtr->shader.vao);
            if (objPtr->shader.faces.size() > 0)
                glDrawElements(GL_TRIANGLES, objPtr->shader.vertexCount, GL_UNSIGNED_INT, 0);
            else
